//A component for showcasing a gallery of images with a title text that displays over them when hovered
import { hobbies } from  "../data/index.json"

export default function GaleryComponent() {
  return (
    <div className=" m-4 h-screen grid grid-cols-3 gap-1 jusitfy-evenly" >
        {hobbies.map((hobby, idx) => 
        <div key={idx} className="relative group overflow-hidden border-6 border-rust-neon m-2">
            <img
              src= {`./hobbies/${hobby.image}`}
              alt={hobby.name}
              className="w-full h-128 object-cover object-bottom-right transition-transform duration-500 group-hover:scale-110"
            />
            <div className="absolute inset-0
                 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
              <h1 className="mt-30 text-2xl text-center h-auto font-bold text-sand-protocol ">{hobby.name}</h1>
              <p className="mx-10 text-2xl text-center  text-sand-protocol ">{hobby.description}</p>
            </div>
        </div>
        )}
    </div>
  )
}
